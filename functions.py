...
def updateNamesOfFilesInDirectory(fileDirectoryFolder, fileDestination, customPrefix): #this process gets the source from the user, processes it, and sends to destination
    ...
    for fileName in os.listdir(path): #
        try:
            ...
            newFileName = fileDestination + "\\" + prefixProcessor(customPrefix) + preFix + "_" + str(extractedFileName) + extractedFileNameExtension # combine the file name and put the directory in front of the file name so it sends the file to that direction
            shutil.copy2(fileName, newFileName ) #copy the file from the source and send the file processed to the destination        
            print(newFileName + " succesfully copied.") #tells user that the file has been copied succesfully
        except:
            #this is to handle errors and to inform the user which file has failed to copy
            print("")
            print("*******Error********")
            print(newFileName + " failed to copy!") #tells user that the file has failed to copy
            print("")
...
def prefixProcessor(prefix): #ensures that "_" is not added to filename unnecessarily
    if prefix == "":
        prefix = ""
        return prefix
    else:
